  ...
  <div class="community_wrap fullscreen bg-gray">
    ...
    <div class="content">
      <van-form validate-first @submit="handleSubmit">
        <van-field class="row" v-model="form.data.name" name="name" :label="$t('community.tags6')"
          :placeholder="$t('community.tags7')" :rules="[
            {
              validator: (val) => !!val.length,
              message: $t('community.tags7'),
            },
          ]" />
        <van-field class="row" v-model="form.data.comName" name="comName" :label="$t('community.tags8')"
          :placeholder="$t('community.tags9')" :rules="[
            {
              validator: (val) => !!val.length,
              message: $t('community.tags9'),
            },
          ]" />
        <van-field disabled :value="form.data.addressText" class="row selectAddress" name="address"
          :label="$t('community.tags10')" :placeholder="$t('community.tags11')" @click="selectAddress"></van-field>
        <van-field class="row" v-model="form.data.phone" name="phone" :label="$t('community.tags12')" type="tel"
          :placeholder="$t('community.tags13')" :rules="[
            {
        ...
        <div style="margin: 25px 15px 0">
          <van-button round block type="info" native-type="submit" :disabled="submitDisable" color="#1B2945">{{
              $t("logout.submit")
          }}
          </van-button>
        </div>
              ...
              create(this.form.data)
                .then((res) => {
                  if (res.data.code) {
                    ...
                    this.$router.push("/my/community/create/success")
                  }
                })
                .catch(() => {
                  this.$toast.fail({
