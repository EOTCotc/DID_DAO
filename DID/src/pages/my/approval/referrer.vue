<template>
  <!-- 推荐人信息 -->
  <van-overlay :show="show" :title="referrer.name" @click="toggle(false)">
    <div class="referrer_wrap" @click.stop>
      <van-cell class="title" :title="referrer.name">
        <van-icon name="cross" size="14px" @click.stop="toggle(false)" />
      </van-cell>
      <van-row class="item-row">
        <van-col span="8" class="name">{{ $t("community.tags8") }}</van-col>
        <van-col span="16" class="value">{{
          referrer.refCommunityName
        }}</van-col>
      </van-row>
      <van-row class="item-row">
        <van-col span="8" class="name">{{ $t("approval.tags6") }}</van-col>
