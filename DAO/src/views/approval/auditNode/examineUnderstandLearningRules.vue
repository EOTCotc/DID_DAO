<template>
  <div>
    ...
    <footer ref="footer">
      ...
      >
        {{ $t("learningRules2.btnmsg") }}</van-button
      >
    </footer>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      if (
        this.$refs.rulesBox.scrollHeight == this.$refs.rulesBox.clientHeight ||
        this.$refs.rulesBox.offsetHeight == this.$refs.rulesBox.scrollHeight
      )
        this.flag = false;
    });
