<template>
  <div class="container">
    <page-header :title="$t('arbitrationcase.title')" />
    <div class="content">
      <ul v-if="list.length != 0">
        <li v-for="(item, index) in list"
          ...
          <div class="details">
            <div>
              <span>{{ item.messageType == 0 ? $t('arbitrationcase.status1') : $t('arbitrationcase.status2') }}</span>
...
export default {
  ...
  methods: {
    ...
    // 消息列表的时间
    msgListTimes(val) {
      let days = this.$t('arbitrationcase.days')
      let now = new Date(),
      ...
      if (nowM == lastM) {
        ...
        if (nowD == lastD) {
          ...
          if (lastT > nextWeek) {
            ...
            fomatd =
              lastM +
              this.$t('arbitrationcase.month') +
              lastD +
              this.$t('arbitrationcase.d')
          }
        }
      } else {
        if (nowY == lastY) {
