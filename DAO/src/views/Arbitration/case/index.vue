      ...
      <div class="content">
        <ul class="list"
          ...
          >
            ...
            <!-- 原被告信息 -->
            <van-row>
              <van-col class="lf" :span="12" @click="go('/user/arbitration/case/personnelInfo', { id: item.plaintiffId, type: 'plaintiff' })">
                <div class="identity_wrap">
                  <img v-if="item.winner === 'plaintiff'" src="../../../assets/imgs/huangguan.png" alt="" class="img">
                  原告
                </div>
                <div class="user">
              ...
              <van-col class="rt" :span="12" @click="go('/user/arbitration/case/personnelInfo', { id: item.defendantId, type: 'defendant' })">
                <div class="identity_wrap">
                  <img v-if="item.winner === 'defendant'" src="../../../assets/imgs/huangguan.png" alt="" class="img">
                  被告
                </div>
                <div class="user">
                  <span class="text">（卖家）</span>
          ...
          if (!res.data.code) {
            const data = res.data.items.map(item => {
              if (item.status > 1) {
                ...
                if (item.status === 2 && item.voteStatus === 1) {
                  // 胜诉方为原告
                  item.winner = 'plaintiff'
                }
                if (item.status === 3 && item.voteStatus === 2) {
                  // 胜诉方为被告
                  item.winner = 'defendant'
                }
              }
